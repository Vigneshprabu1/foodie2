...
export class SignupComponent implements OnInit {
  providerDetails={"username":"", "email":"","mobile_number":"","address":"","password":"","security_question":"","security_answer":""};
  ...
  signup(){
    console.log(this.providerDetails.security_answer);
    ...
    if(this.providerDetails.password==this.providerDetail.confirmpassword){
      ...
      //console.log(this.providerDetails);
      this.data.postDetails(this.providerDetails,'/users').then((result)=> {
        console.log("success");
      },(err)=>{
...
phonenoValidate():boolean{
    for(var i=0;i<this.listData.length;i++){
      if(this.providerDetails.mobile_number==this.listData[i].mobile_number){
        console.log((this.providerDetails.mobile_number==this.listData[i].mobile_number));
        //number match
        this.count++;
      }
    }
