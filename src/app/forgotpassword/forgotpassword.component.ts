    ...
    //console.log(this.data.uid);
      ...
      if (this.responseData) {
        ...
        for(var i=0;i<this.listData.length;i++){
          if(this.storage.get('vicky_phno')==this.listData[i].mobile_number){
            this.question=this.listData[i].security_question;
            this.answer=this.listData[i].security_answer;
            this.username=this.listData[i].username;
            this.storage.set('user_id',this.listData[i].id);
